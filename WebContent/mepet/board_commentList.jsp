...
<body>
	...
	<section>
		<br/>
		<a href="board_commentList.jsp"><h1 align="center" style="margin-bottom: 0; color: #3e6383;">펫시터 이용후기</h1></a>
		<br/>
		<div align="center">
		<button class="button" onclick="window.location.href='board_sitterList.jsp'">펫시터구하기</button> &nbsp; &nbsp; 
		<button class="button" onclick="window.location.href='board_commList.jsp'">미펫 커뮤니티</button>
		<br/><br/><br/>
		<button class="listButt" onclick="window.location.href='board_commentWriteForm.jsp?pageNum=<%=pageNum%>'">후기작성하기</button>
		</div>
		<br/>
			...
			<% 	}else{ //글이 있을 때 
				...
				for(int i = 0; i < getConmmentList.size(); i++){
					...
					if(comment.getRe_step() > 0){	//답글일 때 %>
						<tr>
							<td><%= number-- %></td>
							<td width="430">
							<a href="board_commentContent.jsp?num=<%=comment.getNum()%>&pageNum=<%=pageNum%>&count=<%=count%>&re_step=<%=comment.getRe_step()%>">
							 &nbsp; &nbsp;
							 <img src = "adminImg/rewriteIcon.png" width="15" /> [답글]
							 <%= comment.getSubject() %></a>
							</td>
							<td><%= comment.getSitter_name() %></td>
							<td><%= sdf.format(comment.getReg()) %></td>
							<td><%= comment.getReadcount() %></td>
						 	<%if(admin != null){ %> 
								<td><button onclick="window.location.href='board_commentRDelete.jsp?num=<%=comment.getNum()%>'">삭제</button></td>
							<% } %>
				...
				<% 	}else{ //새글 일 때
						...
						<tr>
							<td><%= number-- %></td>
							<td width="430">
							<h3><a href="board_commentContent.jsp?num=<%=comment.getNum()%>&pageNum=<%=pageNum%>&count=<%=count%>"><%= comment.getSubject() %></a></h3>
							...
							<td><%= comment.getReadcount() %></td>
						<%if(admin != null){ %> 
							<td><button onclick="window.location.href='board_commentDelete.jsp?num=<%=comment.getNum()%>'">삭제</button></td>
							<% } %>
		...
		<%//페이지뷰어 만들기
			if(count > 0){
				...
				<a class="view" href="board_commentList.jsp?pageNum=<%=startPage-1%>"> &lt; </a>
				<%}
				//5. 페이지 반복해서 뿌려주기
				for(int i = startPage; i <= endPage; i++){%>
				<a class="view" href="board_commentList.jsp?pageNum=<%=i%>"> &nbsp; <%=i%> &nbsp; </a>
				<%}
				//7. endPage가 pageCount보다 작으면 '>'
				if(endPage < pageCount){%>
				<a class="view" href="board_commentList.jsp?pageNum=<%=endPage+1%>"> &gt; </a>
				<%}
			}%>
			<br/><br/>
